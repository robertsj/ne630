% Lesson 06: student-facing, board-supported handout.
% This version deliberately omits the Lesson 05 review and reorganizes the
% retained physics as H-1 -> threshold reactions -> U-235 resonances -> SLBW.
...
\centering
\IfFileExists{figures/h1_xsec.pgf}{%
  \resizebox{0.98\linewidth}{!}{\input{figures/h1_xsec.pgf}}%
}{%
...
section changes by a factor of \MathBlank[0.72in]{1/10}.
...
\centering
\IfFileExists{figures/u238_threshold.pdf}{%
  \includegraphics[width=0.98\linewidth]{figures/u238_threshold.pdf}%
}{%
...
reaction channel is \RevealBlank[1.45in]{closed below $E_{\mathrm{th}}$}.
...
\begin{tikzpicture}[
  ...
  % Zero incident-neutron energy is the n + U-238 threshold.
  ...
  % Bound excited states below the neutron threshold.
  ...
  % Compound-nucleus levels above the neutron threshold.
  ...
  % Ground state to threshold: neutron separation energy.
  ...
  % Threshold to one selected positive-energy level.
  ...
  % Ground state to the same level: total compound excitation.
  ...
  % Guides connecting the selected level and ground state to the
  % total-excitation-energy measurement.
...
\centering
\IfFileExists{figures/u235_fission.pgf}{%
  \resizebox{0.98\linewidth}{!}{\input{figures/u235_fission.pgf}}%
}{%
...
\[
 L(E_r)=\MathBlank[0.58in]{1},
 \qquad
 L(E_r\pm\Gamma/2)=\MathBlank[0.58in]{1/2}.
\]
