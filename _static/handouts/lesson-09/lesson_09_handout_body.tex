% Lesson 09: shared student/instructor board-supported handout.
% Content: ne630_lesson09_transcription.tex (handwritten notes, pp. 1--3).
% Structure: lesson_07_handout_body(2).tex.
% Like the supplied body file, this file includes \begin{document}/\end{document}
% and is intended to be read after the course handout preamble.
%
% DRAFT / SOURCE NOTES
% - The source's scope is no absorption, not merely no resonance absorption.
% - The density is defined below per unit energy; the source's literal
%   "between E and dE" wording is not reused. The E to E+dE interval appears
%   explicitly in the source's path-length and reaction-rate definitions.
% - phi_fast is an explicitly declared label for the fast-range approximation;
%   the transcription flags its original subscript (read as g=1) as uncertain.
% - The source uses bar-nu in the general balance and nu in the separately
%   written fission-strength integral. That distinction is retained here.
% - The source's phi=C/E and phi=q/[xi Sigma_s(E) E] are both retained.
%   Their compatibility is a checkpoint, not a silently supplied hypothesis.
% - The inelastic-scattering figure and OpenMC activity remain board prompts;
%   no missing data, illustration, or example has been invented.
% - Reading location: the supplied Project lesson_09.md specifies Section 3.4.
...
% =================== PAGE 1: DEFINITIONS AND BALANCE ===================
...
% ================== PAGE 2: FAST AND SLOWING-DOWN FORMS ==================
\begin{handoutcolumns}
...
$$
\MathBlank[0.45in]{E}<E'<\MathBlank[0.55in]{E/\alpha}.
$$
\RuledLines{1}
\end{checkpoint}
