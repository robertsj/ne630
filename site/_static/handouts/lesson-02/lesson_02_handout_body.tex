...
\begin{tikzpicture}[x=0.0305\linewidth,y=0.23cm,>=Latex]
  ...
  \SolutionFigureContent{%
  \draw[thick,KSUPurple]
    plot[smooth] coordinates {(0.5,0.2) (1.2,2.0) (2.2,5.2) (3.6,7.0)
      (6.0,8.0) (10.0,8.55) (15.0,8.79) (20.0,8.45) (28.0,7.6)};
  \draw[KSUTeal,->,thick] (3.2,5.8) -- (8.2,8.1);
  \draw[KSUOrange,->,thick] (25.8,7.75) -- (19.3,8.42);
  \draw[KSUGray!65,dashed] (15,0) -- (15,8.79);
  \node[above,align=center] at (15,8.79) {$^{56}\mathrm{Fe}$\\$\approx 8.792\unit{MeV}$};
  \node[KSUTeal,above] at (5.1,6.9) {\RevealBlank[0.55in]{fusion}};
  \node[KSUOrange,above] at (23.2,8.25) {\RevealBlank[0.55in]{fission}};
  }
\end{tikzpicture}
...
% Solution cue: fusion and fission both move products toward larger B/A;
% the broad maximum reflects the competition of short-range nuclear binding
% with Coulomb repulsion and surface effects.
...
\scriptsize
\textbf{1.}\ \RevealBlank[1.28in]{fission}\hfill
\textbf{2.}\ \RevealBlank[1.28in]{fusion}\par
\textbf{3.}\ \RevealBlank[3.00in]{radioactive decay / nuclear battery}
\normalsize
\end{fixedbox}
...
\begin{tikzpicture}[
  ...
  % Horizontally elongated nucleus with a vertical "neck"
  \node[
...
\RevealBlank[0.46in]{2--3} neutrons (about
\RevealBlank[0.46in]{2.5} on average in the board sketch).

\smallskip
The split occurs within roughly \RevealBlank[0.66in]{$10^{-17}$ s}; prompt
emissions are complete within roughly \RevealBlank[0.66in]{$10^{-7}$ s}
...
\begin{tikzpicture}[x=0.032\linewidth,y=0.032\linewidth,>=Latex]
  ...
  \SolutionFigureContent{%
  \draw[thick,KSUPurple] plot[smooth] coordinates {(0.4,0.5)(3,3.1)(6,7.0)(9,11.2)(11,14.2)};
  \node[rotate=49,KSUPurple] at (7.6,9.4) {stability};
  \fill[KSUOrange] (5.0,8.1) circle (1.7pt);
  \fill[KSUOrange] (7.1,10.9) circle (1.7pt);
  \draw[KSUOrange,->] (5.0,8.1)--(5.8,7.1);
  \draw[KSUOrange,->] (7.1,10.9)--(7.9,9.8);
  }
\end{tikzpicture}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\linewidth}
...
\begin{tikzpicture}[x=0.031\linewidth,y=0.034\linewidth]
  ...
  \SolutionFigureContent{%
  \draw[thick,KSUPurple] plot[smooth] coordinates {(1,0.3)(2.5,2)(4,8.4)(5.2,9.2)(6.3,5.4)(7.2,2.4)(8,4.0)(9.2,8.8)(10.4,9.6)(11.7,2.0)(12.4,0.4)};
  }
\end{tikzpicture}
\end{minipage}
...
% Solution cue: the fragments lie on the neutron-rich side of the stability
% line and move toward stability mainly through beta-minus decay, accompanied
% by delayed gamma emission and, for a small subset, delayed neutrons.
...
\RevealBlank[1.40in]{antineutrino energy}
\end{checkpoint}

\begin{boardbox}{Reading completion: fuel vocabulary}
\scriptsize
\ifhandoutsolutions
\textbf{Fissionable:} can undergo neutron-induced fission at some neutron
energy. Examples: ${}^{235}$U, ${}^{238}$U, ${}^{239}$Pu.\\[2pt]
\textbf{Fissile:} fissions with low-energy neutrons and can support a chain
reaction. Examples: ${}^{233}$U, ${}^{235}$U, ${}^{239}$Pu, ${}^{241}$Pu.\\[2pt]
\textbf{Fertile:} converts by neutron capture and decay to a fissile nuclide:
${}^{232}$Th$\to{}^{233}$U and ${}^{238}$U$\to{}^{239}$Pu.
\else
...
% Solution cue: for beta-minus decay, the electron bookkeeping is already
% embodied in neutral-atom masses; neutrino masses are neglected here.

\begin{takeawaybox}[Fission-energy scale]
The board estimate is roughly \RevealBlank[0.66in]{$200$ MeV} released per
thermal fission of ${}^{235}$U, with about
\RevealBlank[0.72in]{$199$ MeV} recoverable. Most prompt energy appears as
fragment kinetic energy; each prompt neutron carries roughly
\RevealBlank[0.56in]{$2$ MeV}.
\end{takeawaybox}
...
\RevealBlank[0.54in]{$6\%$} of the pre-shutdown power:
\[
 P_{\mathrm{decay}}(0^+)\approx\MathBlank[0.62in]{0.06}\,P_0.
\]
...
% Solution cue: k counts the neutrons that actually populate the next
% generation, after leakage and non-fission absorption; it is therefore not
% the same quantity as the average number emitted in one fission.
...
\midrule
$k<1$ & \RevealBlank[0.82in]{subcritical} & \RevealBlank[1.22in]{decreases}\\[2pt]
$k=1$ & \RevealBlank[0.82in]{critical} & \RevealBlank[1.22in]{constant}\\[2pt]
$k>1$ & \RevealBlank[0.82in]{supercritical} & \RevealBlank[1.22in]{increases}\\
\bottomrule
\end{tabularx}
\end{boardbox}
...
\[
\begin{aligned}
 P(t)&=\MathBlank[0.52in]{E_f}\,\MathBlank[0.70in]{R_f(t)},\\[-1pt]
 \frac{P(t)}{P(0)}&\approx\MathBlank[1.05in]{\frac{n(t)}{n_0}}.
\end{aligned}
\]
